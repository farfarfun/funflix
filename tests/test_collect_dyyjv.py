...
def _source(**values) -> Source:
    return Source(
        ...
        url="https://bbs.dyyjv.com/d/48861",
        identifier="48861",
...
async def test_collects_daily_pages_and_backfills() -> None:
    index = _payload(
        ...
        "电影云集日更合集",
        "".join(
            f'<p><a href="https://bbs.dyyjv.com/d/{value}">每日</a></p>'
    ...
    def daily(value: int) -> dict:
        return _payload(
            ...
            '<p>百度网盘<br><a href="https://pan.baidu.com/s/1shared?pwd=dyyj">链接</a></p>'
    ...
    collector = DYYJVCollector(client)
    source = _source()
    ...
    assert detect_source(source.url) == (SourceType.FORUM, "48861")
